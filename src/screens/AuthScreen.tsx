...
export const AuthScreen = ({ onLogin }: { onLogin: () => void }) => {
  ...
  // In-App Google Account Picker Sheet
  ...
  const handleGoogleSignInClick = async () => {
    ...
    try {
      ...
      console.warn("Primary Google Sign-In failed, opening In-App Picker:", err);
      // If user cancelled, don't show picker
      if (err?.code === 'auth/popup-closed-by-user' || err?.message?.includes('cancelled')) {
        setLoading(false);
        return;
      }
      // If developer error (SHA-1 not registered in Firebase Console), fallback to In-App Account Picker
  ...
  return (
    <div className="relative min-h-[100dvh] w-full bg-[#F3F4F6] overflow-x-hidden flex flex-col items-center select-none">
      ...
      {/* 1. Top Deep Blue Background (Exact Jetpack Compose Color: 0xFF1E4665, Height 360px) */}
      <div
      ...
      {/* In-App Google Account Selection Sheet */}
      {showGooglePicker && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl max-w-md w-full p-6 shadow-2xl animate-in slide-in-from-bottom-6 sm:zoom-in-95 duration-250 border border-slate-100">
            ...
            <div className="py-3 divide-y divide-slate-100">
              ...
              {/* Enter / Type Google Account */}
              {showAddGoogleInput || savedAccounts.length === 0 ? (
                <div className="pt-3 space-y-2">
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:border-[#1E4665]"
                      autoFocus
                    />
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 bg-[#1E4665] text-white text-xs font-bold rounded-xl shadow-md"
                    >
