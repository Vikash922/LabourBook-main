...
import {
  signInWithEmail,
  signUpWithEmail,
  signInWithGoogleInApp,
  checkGoogleRedirectResult,
  sendPasswordReset
} from '../services/firebaseAuth';
import { useLabor } from '../store/laborStore';

const SAVED_GOOGLE_ACCOUNTS_KEY = 'laborbook_saved_google_accounts';
...
export const AuthScreen = ({ onLogin }: { onLogin: () => void }) => {
  ...
  // In-App Google Account Picker Sheet State
  const [showGooglePicker, setShowGooglePicker] = useState(false);
  const [googleCustomEmail, setGoogleCustomEmail] = useState('');
  const [showAddGoogleInput, setShowAddGoogleInput] = useState(false);
  const [savedAccounts, setSavedAccounts] = useState<SavedAccount[]>(() => {
    try {
      const saved = localStorage.getItem(SAVED_GOOGLE_ACCOUNTS_KEY);
      if (saved) return JSON.parse(saved);
    } catch {}
    return [
      { name: 'Vikash', email: 'vikash922@gmail.com' }
    ];
  });
  ...
  const handleSignUp = async (e?: React.FormEvent) => {
    ...
    setErrorMsg(null);
    try {
      const displayName = contractorName.trim() || undefined;
  ...
  // In-App 1-Tap Google Account Selection
  const handleSelectGoogleAccount = async (targetEmail: string, targetName?: string) => {
    setLoading(true);
    setErrorMsg(null);
    setShowGooglePicker(false);
    try {
      const user = await signInWithGoogleInApp(targetEmail, targetName);
      // Save account for quick 1-tap re-login in future
      const updated = [
        { name: targetName || targetEmail.split('@')[0], email: targetEmail },
        ...savedAccounts.filter((a) => a.email.toLowerCase() !== targetEmail.toLowerCase())
      ];
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col justify-between bg-gradient-to-b from-[#163D59] via-[#1E4E70] to-[#F1F5F9] select-none">
      ...
      <div className="w-full max-w-md mx-auto px-4 pb-4">
        <div className="bg-white rounded-3xl shadow-2xl shadow-slate-900/15 border border-slate-100 overflow-hidden">
          ...
          <div className="px-6 pb-6 pt-2">
            ...
            {/* Google Sign In Button (Opens In-App Account Picker) */}
            <button
              type="button"
              onClick={() => setShowGooglePicker(true)}
              disabled={loading}
              className="w-full py-3 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold rounded-xl transition active:scale-[0.98] flex items-center justify-center gap-2.5 text-sm shadow-2xs cursor-pointer"
            >
      ...
      {/* In-App Google Account Picker Bottom Sheet */}
      {showGooglePicker && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl max-w-md w-full p-6 shadow-2xl animate-in slide-in-from-bottom-6 sm:zoom-in-95 duration-250 border border-slate-100">
            ...
            <div className="py-3 divide-y divide-slate-100">
              {savedAccounts.map((acc, index) => (
                <button
                  ...
                  className="w-full py-3 px-2 flex items-center gap-3.5 hover:bg-slate-50 active:bg-slate-100 rounded-2xl transition text-left cursor-pointer"
                >
                  <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-500 text-white font-bold text-sm flex items-center justify-center shadow-xs shrink-0">
              ...
              {/* Add / Type Another Google Account Option */}
              {showAddGoogleInput ? (
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (googleCustomEmail.trim()) {
                          handleSelectGoogleAccount(googleCustomEmail.trim());
                        }
                      }}
                      className="flex-1 py-2 bg-[#1656D6] text-white text-xs font-bold rounded-xl"
                    >
                      Sign In
                    </button>
                  </div>
            ...
            <div className="pt-2 text-center">
              <p className="text-[10px] text-slate-400">
                To continue, Google will share your name, email address, and language preference with LabourBook.
              </p>
            </div>
          </div>
        </div>
